...
impl YoutubeRestHandler {
    ...
    ) -> Result<ConfigUpdateResponse, RestError> {
    ...
    pub async fn handle_stream_request(
        ...
        Err(RestError::NotImplemented(
            "Direct streaming not yet implemented".to_string(),
        ))
    }
...
mod tests {
    ...
    fn test_rest_error_status_codes() {
        assert_eq!(
            RestError::Configuration("test".to_string()).status_code(),
            400
        );
        assert_eq!(
            RestError::Authentication("test".to_string()).status_code(),
            401
        );
        assert_eq!(RestError::NotFound("test".to_string()).status_code(), 404);
        assert_eq!(
            RestError::NotImplemented("test".to_string()).status_code(),
            501
        );
