use super::{ClientProvider, ClientProviderV3, ClientProviderV4, PluginInfo, YoutubeConfig};
use crate::client::{
    AndroidClient, IosClient, MusicClient, TvClient, WebClient, WebEmbeddedClient,
};
use crate::{Client, YoutubeAudioSourceManager, YoutubeSource, YoutubeSourceOptions};
...
impl YoutubePluginLoader {
        ...
        if let Some(config) = &self.config {
            if !config.is_enabled() {
                ...
                return Err(crate::YoutubeError::ConfigurationError(
                    "YouTube source is disabled".to_string(),
                ));
            }
        }
    ...
    async fn create_source_manager(&self) -> crate::Result<YoutubeAudioSourceManager> {
        ...
        if let Some(_provider) = &self.client_provider {
            ...
            Ok(YoutubeAudioSourceManager::with_options_and_clients(
                options, clients,
            ))
        } else {
    ...
    async fn create_clients(&self, client_names: &[String]) -> crate::Result<Vec<Box<dyn Client>>> {
        ...
        if clients.is_empty() {
            return Err(crate::YoutubeError::ConfigurationError(
                "No valid clients configured".to_string(),
            ));
        }
...
mod tests {
    ...
    async fn test_client_names_default() {
        ...
        assert_eq!(
            client_names,
            vec!["MUSIC", "ANDROID_VR", "WEB", "WEBEMBEDDED"]
        );
    }

    #[tokio::test]
    async fn test_client_names_custom() {
        let config = YoutubeConfig::new().set_clients(vec!["WEB".to_string(), "MUSIC".to_string()]);
        let loader = YoutubePluginLoader::new(Some(config));
        let client_names = loader.get_client_names();
