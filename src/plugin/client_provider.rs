...
impl ClientReference {
    ...
    pub fn get_client(&self, options: ClientOptions) -> crate::Result<Box<dyn Client>> {
        debug!(
            "Initialising client {} with options {:?}",
            self.name, options
        );
...
impl ClientProvider for ClientProviderV3 {
    fn get_clients(
        ...
        if resolved.is_empty() {
            return Err(crate::YoutubeError::ConfigurationError(
                "No valid clients could be created".to_string(),
            ));
        }
    ...
    fn get_client_references(&self) -> Vec<ClientReference> {
        ...
        vec![
            ClientReference::new("MUSIC", |opts| {
                Ok(Box::new(MusicClient::with_options(opts)))
            }),
            ClientReference::new("WEB", |_opts| Ok(Box::new(WebClient::new()?))),
            ClientReference::new("WEBEMBEDDED", |opts| {
                Ok(Box::new(WebEmbeddedClient::with_options(opts)))
            }),
            ClientReference::new("ANDROID", |opts| {
                Ok(Box::new(AndroidClient::with_options(opts)))
            }),
            ClientReference::new("ANDROID_VR", |opts| {
                Ok(Box::new(AndroidClient::vr_with_options(opts)))
            }),
            ClientReference::new("ANDROID_MUSIC", |opts| {
                Ok(Box::new(AndroidClient::music_with_options(opts)))
            }),
            ...
            ClientReference::new("TVHTML5EMBEDDED", |opts| {
                Ok(Box::new(TvClient::html5_embedded_with_options(opts)))
            }),
        ]
    }
}
...
mod tests {
    ...
    fn test_client_provider_v3_default_clients() {
        ...
        assert_eq!(
            default_clients,
            vec!["MUSIC", "WEB", "ANDROID_VR", "WEBEMBEDDED"]
        );
    }
    ...
    fn test_simple_options_provider() {
        ...
        assert_eq!(
            provider.get_options_for_client("NONEXISTENT"),
